...
- [x] 핵심 테이블과 제약이 추가 전용 migration으로 재현된다.
- [x] 활성 집 하나 제약을 위한 부분 고유 인덱스가 DB에 있다.
- [x] 정원 4와 출석 100 설정이 idempotent seed로 정의되고 원격 DB에 확인됐다.
- [x] 생성 타입이 실제 원격 스키마에서 생성됐고 typecheck가 통과한다.
